...
class DIDOrchestrator {
  ...
  async run(classification, routing, hardState, options = {}) {
    ...
    const plannerRaw = await callModel(
      ...
      [],
      null,
      { expectJson: false, signal, onChunk: wrapChunk('architecturePlanner') }
    );
    ...
    // Preserve proof artifact; reviewer phase intentionally sees planner output in step 2.
    const protectedHashes = computeContentHashes(plannerRaw);

    // Stage 2 — Reviewer Round 1
    const reviewerRaw = await callModel(
      'componentPlanner',
      buildReviewerPrompt(taskContext, plannerRaw),
      { classification, routing },
      hardState,
      [],
      null,
      { expectJson: false, signal, onChunk: wrapChunk('componentPlanner') }
    );
    ...
    const tiebreakerRaw = await callModel(
      ...
      [],
      null,
      { expectJson: false, signal, onChunk: wrapChunk('tiebreaker') }
    );
