...
### Section 1A — Startup Determinism (Gate 0 Lock)
- Gate 0 startup steps are exhaustive until explicit human `GOOD`.
- During Gate 0, do not run exploratory discovery commands (`rg`, `grep`, `find`, `ls`, `tree`, heading scans) unless explicitly listed in the active runbook.
- If any document appears to require extra startup checks, report the conflict in one sentence and halt for human instruction.

### Gate 0 Stop Point Clarification
- Gate 0 runs in two phases:
  1. Phase A (mandatory before pause): read AGENTS, `projecttracking.md`, `BUGS.md`, compute/report Base nhash, then STOP and wait.
  2. Phase B (only after human `GOOD`): run `python3 bin/handshake.py --status`, then run the required `graph_search(...)` query.
- Do not execute Phase B before human `GOOD`.
...
On `approved`: update `projecttracking.md`, `BUGS.md`, and changelog/state artifacts
...
9. **Sync State** (`projecttracking.md`, `BUGS.md`, `.mbo/sessions/NEXT_SESSION.md`).
...
4. `.mbo/sessions/NEXT_SESSION.md` is generated output only. Manual edits are
   prohibited.
...
8. `python3 bin/validator.py --all` MUST fail when:
   ...
   - `.mbo/sessions/NEXT_SESSION.md` is stale relative to governance files.
