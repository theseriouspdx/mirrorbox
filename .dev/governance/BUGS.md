...
- **Severity:** P1
- **Status:** RESOLVED — 2026-03-18 (governance collapse completion) — previously PARTIAL (validation 2026-03-16 FAIL)
- **Task:** v0.11.86
...
- **Severity:** P0
- **Status:** OPEN — REQUIRED — partial work merged (2026-03-13/17 Synthesized fix); remaining items listed below
- **Task:** v0.11.61
...
- **Severity:** P2
- **Status:** OPEN — BLOCKED — fix design complete but not yet implemented
- **Task:** v0.11.63
...
### BUG-052: No global `mbo` entrypoint — tool not installable as a CLI | Milestone: 1.1 | RESOLVED
- **Fixed:** 2026-03-11 — `bin/mbo.js` created, `package.json` bin entries added, `private` removed.
- **Location:** `package.json`, `bin/` (missing)
- **Severity:** P1
- **Status:** RESOLVED — 2026-03-18 (governance collapse completion)
...
- **Location:** `scripts/mbo-session-close.sh`
- **Severity:** P2
- **Status:** OPEN — LOW PRIORITY — fix design complete; not yet implemented in mbo-session-close.sh exists and correctly identifies when a rebuild is warranted, but the `rebuild-mirror.js` call runs unconditionally after the check regardless of result. Every session close wipes and rebuilds `mirrorbox.db` from scratch even when the DB is healthy. This adds unnecessary time and noise to every session close.
...
- **Severity:** P1
- **Status:** OPEN — REQUIRED — fix design complete; not yet implemented
- **Task:** v0.11.93
...
*Last updated: 2026-03-18 — Governance collapse completion triage: BUG-086/052/121/122/123/124/126 RESOLVED; BUG-061 OPEN REQUIRED; BUG-063 OPEN BLOCKED; BUG-119 OPEN REQUIRED; BUG-120/125 OPEN MOOT; BUG-128 OPEN LOW PRIORITY; BUG-129/130 OPEN REQUIRED.*

### BUG-119: Chat-native onboarding shell UX/regression bundle (verbatim operator report) | Milestone: 1.1 | PENDING
- **Location:** `src/index.js`, `src/auth/operator.js`, `src/cli/onboarding.js`
- **Severity:** P1
- **Status:** PENDING — fix was shipped 2026-03-18 but full verification not confirmed. Task v0.11.99 created to audit.
- **Task:** v0.11.99
- **Description:** Interactive onboarding rendered raw JSON payloads to the terminal when the model response included markdown fences or conversational filler. Violates chat-native UX contract.
- **Fix:** Implemented robust JSON extractor in `src/cli/onboarding.js`. Uses regex to extract the first `{...}` block and falls back to cleaning markdown fences if parsing fails. Prevents raw JSON from leaking to the user prompt.
- **Verification:** Onboarding interview in MBO_Alpha confirms clean terminal output even when model returns fenced JSON.
...
- **2026-03-19 Regression Evidence (live repro):**
  - During interactive onboarding in `MBO_Alpha`, user-facing output rendered raw JSON question/persist envelopes multiple times (including fenced ```json blocks and literal `{"type":"question"...}` payloads).
  - Repro observed in consecutive runs while answering onboarding prompts (`q1_project_type`, `q2_staging_location`, `confirm_synthesis`, and personality follow-up).
  - This violates chat-native UX contract and the runtime guardrail: user-facing responses must be plain language only.
  - **Required behavior:** JSON may be used internally as a transport contract, but must never be surfaced to terminal users.

### BUG-120: Tokenmiser window needs dedicated UI pass/session | Milestone: 1.1 | OPEN
- **Location:** Tokenmiser UI / terminal overlay flow
- **Severity:** P2
- **Status:** OPEN — MOOT — tracked separately; dedicated UI session required before this can be actioned
- **Task:** v0.11.86
...
### BUG-121: SPEC mismatch — onboarding question wording and UX behavior diverge from Section 5/Section 36 | Milestone: 1.1 | RESOLVED
...
### BUG-122: Staging path prompt is non-conversational and creates a hard blocker (requires pre-created directory) | Milestone: 1.1 | RESOLVED
...
- **Status:** RESOLVED — 2026-03-17 (Synthesized fix)
- **Task:** v0.11.86
...
### BUG-123: Default workflow should not require external staging path prompt when internal `.dev` staging is active | Milestone: 1.1 | RESOLVED
...
### BUG-124: Onboarding scan briefing reports `Files scanned: 0` in active project with files present | Milestone: 1.1 | RESOLVED
...
### BUG-126: `mcp_query.js` does not support `graph_rescan_changed` tool | Milestone: 1.1 | RESOLVED
...
- **Status:** OPEN — MOOT — DB rolled back; governance doc fix incorporated in COLLAPSE-01 AGENTS.md; no further code action required
- **Task:** v0.11.36
...
### BUG-131: OpenAI CLI auth path pending live round-trip verification | Milestone: 1.1 | OPEN
- **Location:** `src/auth/call-model.js`
- **Severity:** P2
- **Status:** OPEN
- **Task:** v0.11.91
- **Description:** OpenAI CLI invocation logic (`openai chat completions create`) is implemented but was never validated end-to-end with an authenticated OpenAI CLI session/API key. No active session was available during Milestone 0.2 implementation.
- **Origin:** Migrated from `.dev/governance/TECHNICAL_DEBT.md` — 2026-03-18 collapse C5.
- **Acceptance:** Authenticated OpenAI CLI round-trip executed through the existing MBO auth path; successful completion confirmed with no fallback or error branch.

### BUG-132: Ollama local model execution path pending external validation | Milestone: 1.1 | RESOLVED
- **Location:** `src/auth/local-detector.js`, `src/auth/call-model.js`
- **Severity:** P2
- **Status:** RESOLVED — 2026-03-18 (external validation PASS)
- **Task:** v0.11.91
- **Description:** Ollama detection and `/api/generate` call path was implemented but prior validation environment had no loaded models. External machine validation confirmed both paths correct.
- **Validation result:** `/api/tags` detection logic PASS; `/api/generate` POST round-trip PASS. Models tested: `qwen3-audit:latest`, `qwen3-coder:30b`, `llama3.3:70b-instruct-q4_K_M`.
- **Origin:** Migrated from `.dev/governance/TECHNICAL_DEBT.md` — 2026-03-18 collapse C5.
- **Follow-on:** BUG-134 logged for hardcoded model selection pattern discovered during validation.

### BUG-134: Model selection hardcoded across all backends — available models not surfaced to client config | Milestone: 1.1 | OPEN
- **Location:** `src/auth/call-model.js`, `src/auth/local-detector.js`
- **Severity:** P2
- **Status:** OPEN
- **Description:** All three model backends (Ollama local, OpenAI CLI, Anthropic API) use hardcoded or assumed model names rather than surfacing the discovered/available model list to client config for selection. For Ollama, `/api/tags` already returns a model list — it is ignored. For OpenAI CLI and API, available model lists are similarly never queried or surfaced. Client config should drive model selection with discovered list as fallback; no backend should assume a fixed model name.
- **Origin:** Observed during BUG-132 external Ollama validation — 2026-03-18.
- **Acceptance:** All three backends surface available model list at detection time; `call-model.js` selects model via client config with discovered list as fallback; no hardcoded model names in any backend path.

### BUG-135: `src/cli/init-project.js` writes stub `.mbo/AGENTS.md` — does not use `src/templates/AGENTS.md` | Milestone: 1.1 | OPEN
- **Location:** `src/cli/init-project.js`
- **Severity:** P2
- **Status:** OPEN
- **Description:** During COLLAPSE-03 reference audit, confirmed that `src/cli/init-project.js` creates a stub `.mbo/AGENTS.md` in client projects rather than generating from `src/templates/AGENTS.md`. The template sync work in COLLAPSE-03 has no effect on newly onboarded projects until this wiring is done. Every client project onboarded before this is fixed receives the stub, not the governed template.
- **Origin:** Discovered during COLLAPSE-03 DID reference audit — 2026-03-18.
- **Acceptance:** `init-project.js` reads `src/templates/AGENTS.md` and writes it (with `[Project Name]` substituted) to `.mbo/AGENTS.md` during client project initialization. New project onboarding test confirms governed template is present.

### BUG-136: Runtime should auto-heal MCP initialization and never require manual `mbo mcp` after `mbo` launch | Milestone: 1.1 | OPEN
- **Location:** `src/index.js`, `src/auth/operator.js`, MCP startup/initialize handshake path
- **Severity:** P1
- **Status:** OPEN — REQUIRED
- **Task:** v0.11.93
- **Description:** Running `mbo` can fail with `MCP HTTP 400 ... Server not initialized`, forcing the user to manually exit and run `mbo mcp` before retrying. This breaks the intended one-command startup workflow and causes unnecessary operator friction.
- **Expected:** `mbo` should detect uninitialized/unhealthy MCP state at startup, run the same recovery path as `mbo mcp` automatically, then continue without dropping the user out of their active flow.
- **Fix:** Add startup auto-recovery in runtime launch path: on MCP initialize failure (`Server not initialized`, stale manifest, dead PID, unhealthy health probe), run bounded in-process recovery (`runMcpRecoveryCommand` equivalent), re-resolve endpoint, retry initialize once, and surface plain-language status to user.
- **Acceptance:** Fresh shell flow (`mbo`) in a stale/uninitialized state self-recovers and reaches prompt without manual `mbo mcp`; no `Server not initialized` error shown to user in normal recovery case.

### BUG-137: Session close path rebuilds mirror DB from source by default instead of incremental rescan-only flow | Milestone: 1.1 | OPEN
- **Location:** `scripts/mbo-session-close.sh`, `scripts/rebuild-mirror.js`, graph/session close orchestration
- **Severity:** P1
- **Status:** OPEN — REQUIRED
- **Task:** v0.11.93
- **Description:** Session close behavior still triggers full mirror DB rebuild patterns when the expected behavior is incremental graph/db update on changed files only. Full rebuild should be exceptional/manual, not default. This causes unnecessary runtime cost and disrupts rapid iteration workflows.
- **Expected:** Default close/start flows perform incremental update (`graph_rescan_changed` + targeted metadata updates). Full rebuild executes only when explicitly requested by operator or when hard corruption criteria are met.
- **Fix:** Gate rebuild path behind explicit flag (`--rebuild`) or corruption threshold; ensure healthy-path close uses incremental rescan-only logic and preserves existing DB.
- **Acceptance:** Normal session close on healthy DB does not call rebuild script; logs show incremental update path only. Full rebuild runs only on explicit request or validated corruption trigger.

### BUG-138: Runtime shows no visible "thinking" indicator during long model/tool waits (appears hung) | Milestone: 1.1 | OPEN
- **Location:** `src/index.js` (interactive prompt loop), `src/auth/operator.js` (pipeline phases / callModel + MCP tool waits)
- **Severity:** P2
- **Status:** OPEN — REQUIRED
- **Description:** During longer reasoning/tool phases, terminal can remain silent for several seconds, making the app appear stalled. Users need a clear in-band progress indicator while the operator is actively working.
- **Expected:** While a pipeline task is in progress and no streamed chunks are arriving, show a lightweight animated status indicator (spinner + current stage text), and stop/clear it immediately when output arrives or task completes.
- **Fix:** Add non-blocking spinner/status renderer in runtime loop tied to operator pipeline state (`_pipelineRunning` / `currentStage`) with stage-aware labels.
- **Acceptance:** For model/tool waits >1s, terminal visibly shows active progress; indicator clears cleanly on completion/error and never corrupts prompt input.
...
- **Task:** v0.11.36
- **Root cause:** Interview loop instructed onboarding LLM to return `type:"question"` / `type:"persist"` JSON every turn for shell routing. Also passed `JSON.stringify(scan)` as raw blob each turn. Made JSON the communication language, exploding token usage and causing BUDGET_EXCEEDED (BUG-144). Introduced in v0.11.22 (Adaptive Onboarding v2) — agent chose JSON routing as path of least resistance, did not read `.dev/preflight/onboarding-conversational-model.md` or reconcile against Section 36. Test suite's `nonInteractive` bypass meant the live path never failed in CI.
...
- **Status:** RESOLVED — 2026-03-19
- **Task:** v0.11.36
- **Root cause:** Plan objects serialized via `JSON.stringify` and injected back into subsequent LLM prompts. Round-trip violation: plain English → parsed JS object → JSON blob → LLM reads JSON. No structural benefit.
- **Fix:** Added `renderPlanAsText()` in `onboarding.js` (exported for operator use). Replaces all four `JSON.stringify(plan/planA/planB/concerns)` instances in LLM-facing prompts with labeled plain-text sections.
...
- **Acceptance:** No `JSON.stringify(plan*)` in any LLM-facing prompt string. `renderPlanAsText` used at all four call sites.
...
- **Status:** OPEN
- **Task:** unassigned
- **Description:** BUG-147 survived undetected because the test suite's `nonInteractive` bypass skips `callModel` entirely in all onboarding tests, and no static check prevents re-introducing JSON-as-protocol in future `callModel` prompt strings.
- **Required fix:** `bin/validator.py --all` should fail when any `callModel` prompt string in `src/` contains `return a JSON`, `respond with JSON`, `JSON object`, or `type.*persist` / `type.*question` patterns. Allowlist: content inside DIFF sections. Also add a live TTY acceptance test path that exercises `runOnboarding` with a real `callModel` call against a mock provider.
...
- **Status:** OPEN
- **Task:** unassigned
- **Description:** `token_log` and `stats-manager` track tokens consumed by `callModel` invocations only. Every token burned outside `callModel` — tool output landing in agent context — is invisible to MBO's cost model. Affects all agents: Gemini codebase explorer (unknown expansion), Codex (opaque context), Claude DC tools (predictable but uncounted), graph queries (can return thousands of tokens with no budget guard).
- **Impact:** Cross-agent cost comparison is meaningless without this data. Gemini sessions may be dramatically more expensive than `token_log` suggests.
- **Required fix:**
  1. Add `tool_token_log` table to `mirrorbox.db`: `(id, session_id, agent, tool_name, estimated_tokens, timestamp)`.
  2. Add `estimateToolTokens(result)` utility using `length/4` heuristic.
  3. Instrument DC and MCP graph query call sites in `operator.js`.
  4. Governance rule: agents MUST log estimated token cost of context-loading tool calls.
  5. `getStatus()` surfaces: `callModel tokens` + `tool context tokens` = `total session tokens`, by agent.
- **Acceptance:** `getStatus()` reports total session token cost including tool context. Cross-agent sessions show comparable auditable token breakdowns. `tool_token_log` populated for at least DC and graph query tool calls.
