...
| v0.11.180 | bug | BUG-177 workflow audit governance-file compliance false failure for docs/e2e-audit-checklist.md | READY | unassigned | - | 2026-03-21 | BUG-177 | Workflow audit no longer reports docs/e2e-audit-checklist.md missing when the file exists; governance compliance passes on rerun |
...
| v0.3.07 | bug | BUG-180 TasksOverlay: scrollable cursor, Enter-to-select, on-select aggregates spec+bugs+projecttracking, synthesis + confirm prompt | READY | claude | - | 2026-03-20 | BUG-180 | Scrollable task list with cursor; Enter aggregates all source docs and shows briefing (goal, assumptions, acceptance, proposed files). User can add context/files, correct assumptions, and must explicitly confirm before task context is activated. |
| v0.3.08 | bug | BUG-181 Audit gate flow: tab auto-switch to [1], pipeline border color, InputBar [AUDIT] prefix on audit_gate stage | READY | claude | - | 2026-03-20 | BUG-181 | TUI responds correctly to audit_gate stage across all three visual indicators |
| v0.3.09 | bug | BUG-182 SystemPanel MCP health: port polling and live health display unverified | READY | claude | - | 2026-03-20 | BUG-182 | SystemPanel correctly polls MCP ports and displays live health status |
| v0.3.10 | bug | BUG-183 PipelinePanel scroll: auto-scroll and user-scroll-lock unverified during live streaming | READY | claude | - | 2026-03-20 | BUG-183 | PipelinePanel auto-scrolls during streaming; user scroll locks auto-scroll until re-engaged |
...
| v0.3.17 | bug | BUG-184 C.purple too dark — blueBright unreadable for labels/borders; fix with hex color | READY | claude | - | 2026-03-20 | BUG-184 | C.purple renders as bright readable purple/violet on dark terminals; all usages updated consistently |
