...
## [1.1.0] — 2026-03-15
### Added
- **Project-Scoped MCP Isolation**: Implemented isolated graph server daemons using hex-suffixed launchd labels and ephemeral ports.
- **Manifest Identity Enforcement**: Operator and `mcp_query.js` now validate both canonical `project_root` and `project_id` hash before connection.
- **Atomic Manifest Lifecycle**: Servers now write PID-labeled manifests and maintain atomic symlinks for discovery.
- **Legacy Daemon Migration**: Automated cleanup of global `com.mbo.mcp.plist` during setup.
