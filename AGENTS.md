...
### 6C. Session End
If "end session" is requested: write timestamped session handoff artifacts, terminate MCP, run Branch Finalization, and output checklist.
...
4. Session handoffs are optional generated timestamped artifacts. Manual edits are prohibited.
5. Session-close may regenerate handoffs from `projecttracking.md` + OPEN/PARTIAL P0/P1 bugs, but it is not required for validator pass/fail.
